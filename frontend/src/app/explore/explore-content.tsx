...
function ExploreContent() {
  ...
  // Reload data whenever page / query / sort / category changes
  useEffect(() => {
    const isPagination = currentPage !== 1 && videos.length > 0;
    loadVideos(currentPage, activeQuery, currentSort, currentCategory, isPagination);
  }, [currentPage, activeQuery, currentSort, currentCategory, loadVideos]);
        ...
        <div className="px-4 pt-4 pb-1">
          <p className="text-[#888] text-sm">
            {total > 0 ? (
              <>
                <span className="text-white font-semibold">{total}</span> result{total !== 1 ? "s" : ""} for{" "}
                <span className="text-[#F5A200]">"{currentCategory}"</span>
              </>
            ) : (
              <>No results for <span className="text-[#F5A200]">"{currentCategory}"</span></>
            )}
          </p>
        </div>
      )}
